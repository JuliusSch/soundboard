...
from database import init_db, get_all_tracks, add_track
from audio_handler import download_audio
...
class SoundboardApp:
    ...
    def on_track_click(self, track):
        print(f"Track clicked: {track[1]}")  # Debug: Print the clicked track

        # Check if the track is already in the selected panel
        if track[1] not in [w.cget("text") for w in self.selected_tracks_frame.winfo_children()]:
            track_label = ctk.CTkLabel(
            ...
            track_label.pack(fill="x", pady=2, padx=5)
            track_label.track = track
            # Bind double-click to play the track
    ...
    def play_track(self, path):
        print(f"Attempting to play: {path}")  # Debug: Print the file path
        ...
        try:
            ...
            print(f"Playing track: {path}")  # Debug: Confirm track is playing
        except Exception as e:
